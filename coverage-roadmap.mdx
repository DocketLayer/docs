...
Ten courts carry `full` coverage: the three federal courts (`fct`, `fca`, `tcc`) and seven provincial courts (`bc_sc`, `bc_ca`, `mb_kb`, `mb_ca`, `nb_kb`, `nb_ca`, `on_ca`). The remaining 15 active courts carry `partial` coverage.
