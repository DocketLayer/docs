...
**Payment** — A single x402 payment covers the batch. The payment transaction must be sized for the full batch (`batch_size × $0.99`). If all cases fail, we settle no payment. If some cases fail, we still charge the full batch amount — x402 payments are atomic and cannot be partially refunded.
