...
| `meta.total_cost_usd` | number | no | `0.99 × meta.batch_size`. The x402 payment covers the full batch upfront. We settle no payment only if all queries fail; if any succeed, we charge the full batch amount. |
